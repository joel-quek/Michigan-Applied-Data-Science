...
def answer_eleven():
    ...
    df = answer_one()
    df['PopulationEstimate'] = df['Energy Supply'] / df['Energy Supply per Capita']
    df['Cont'] = pd.Series(ContinentDict)

    return df.groupby('Cont')['PopulationEstimate'].agg([np.size,np.sum, np.mean, np.std])

...
def answer_ten():
    df = answer_one()
    df['HighRenew'] = pd.Series(dtype='int')
    med = df['% Renewable'].median()
    for i in range(15):
        if df.iloc[i]['% Renewable'] < med:
            df.iloc[i, df.columns.get_loc('HighRenew')] = 0
        else:
            df.iloc[i, df.columns.get_loc('HighRenew')] = 1
    # return df.loc[:, 'HighRenew']
